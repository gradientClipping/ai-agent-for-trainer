...
# ─────────────────────────────────────────────
# SYSTEM PROMPT (unchanged from notebook)
# ─────────────────────────────────────────────
SYSTEM_PROMPT = """
### ROLE & OBJECTIVE
...
def run_evaluation(client, sot_df, responses_df, progress_bar, status_text) -> dict:
    ...
    for num in range(len(q_cols)):
        ...
        status_text.markdown(
            f'<p style="font-size:0.83rem;color:#FFFFFF;">Processing <strong>Question {num+1}</strong>'
            f' of {len(q_cols)} &nbsp;|&nbsp; Type: <strong>{qtype}</strong></p>',
...
def main():
    ...
    with st.sidebar:
        ...
        api_key = st.text_input(
            ...
            type="password",
            placeholder="sk-proj-...",
            help="Used only during this session. Not stored.",
            label_visibility="collapsed",
        )
        if api_key:
            st.markdown(
                '<div class="alert alert-success" style="margin-top:0.4rem;font-size:0.78rem;">API key entered</div>',
                unsafe_allow_html=True,
            )
        else:
            st.markdown(
                '<div class="alert alert-warning" style="margin-top:0.4rem;font-size:0.78rem;">Enter your OpenAI API key to enable scoring</div>',
                unsafe_allow_html=True,
            )
    ...
    if not api_key:
        alert("warning", "Enter your OpenAI API key in the sidebar to enable AI scoring.")
        return
    ...
    if st.button("Start Evaluation", type="primary", use_container_width=True):
        ...
        col_prog, col_stat = st.columns([3, 2])
        with col_prog:
        ...
        start = time.time()
        try:
            results = run_evaluation(client, sot_df, responses_df, progress_bar, status_text)
        except Exception as e:
        ...
        status_text.markdown(
            f'<p style="font-size:0.83rem;color:#22C55E;font-weight:600;">Complete in {elapsed:.0f}s</p>',
            unsafe_allow_html=True,
        )
